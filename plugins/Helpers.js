...
export default defineNuxtPlugin(() => {
  ...
  const formatAmount = (amount, decimal = 2) => {
    return numberWithCommas(
      parseFloat(amount)
        .toFixed(2)
        .toLocaleString("en-US", { maximumFractionDigits: decimal }),
    );
  };
  ...
  const PaymentModeDescription = (mode) => {
    if (mode == 1) return "Check";
    else if (mode == 2) return "Bank Transfer";
    else if (mode == 3) return "E-Wallet";
    else return "Cash";
  };

  const ReceiptTypeDesc = (receiptType) => {
    if (receiptType == 1) return "OR";
    else if (receiptType == 2) return "AR";
    else if (receiptType == 3) return "PR";
    else return "";
  };
  const PettyCashTransTypeDesc = (transtype) => {
    if (transtype == 0) return "Cash In";
    else if (transtype == 1) return "Transfer Out";
    else if (transtype == 2) return "Transfer In";
    else return "";
  };
  ...
  const getNumberRank = (data) => {
    ...
    if (lastChar == "1" && last2Char != "11") return data + "st";
    else if (lastChar == "2" && last2Char != "12") return data + "nd";
    else if (lastChar == "3" && last2Char != "13") return data + "rd";
    else return data + "th";
  };
