            ...
            >
              <template
                ...
                v-if="
                  !(
                    state.isPageLoading ||
                    (state.payables.data && state.payables.data.length === 0)
                  )
                "
              >
                <tr
                  v-for="(payable, index) in state.payables.data"
                  :key="index"
                  class="text-gray-700"
                >
                  <td class="px-4 py-3">
...
async function fetchData() {
  ...
  try {
    const response = await paymentService.getPayables(
      props.clientID,
      state.currentPage,
      state.searchString,
    );
