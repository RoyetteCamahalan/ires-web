...
async function fetchData() {
  ...
  try {
    const response = await projectService.GetAvailableRentalUnits(
      state.currentPage,
      state.searchString,
    );
...
function removeObjectWithId(id) {
  const objWithIdIndex = state.selectedItems.findIndex(
    (obj) => obj.propertyid === id,
  );
...
function submit() {
  ...
  } else state.error = "Please select record.";
}
</script>
