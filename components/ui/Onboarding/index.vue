    ...
    <ModalEmpty :isShow="props.isShow">
      <LoadingSpinner :isActive="state.isPageLoading">
        ...
        <div v-else class="w-full bg-white rounded-lg">
          ...
          >
            <h1 class="text-sm text-blue-600">
              First, we will add a Building. You can manage your building under
              `Buildings` menu
            </h1>
            <UiProperty
              :isOnBoarding="true"
              @onSkipTour="completeTour"
              @modalClose="afterSaveBuilding"
            ></UiProperty>
          </div>
          <div
            v-else-if="state.currentPage == 2"
            class="pt-4 px-4 ease-in-out duration-300"
          >
            <h1 class="text-sm text-blue-600">
              Now, lets add room to your Building. You can manage your building
              under `Buildings` menu then click `Manage`
            </h1>
            <UiPropertyUnit
              :isOnBoarding="true"
              :selectedPropertyID="state.currentBuilding.propertyid"
              @onSkipTour="completeTour"
              @modalClose="afterSaveUnit"
            ></UiPropertyUnit>
          </div>
          <div
            v-else-if="state.currentPage == 3"
            class="ease-in-out duration-300"
          >
            <div class="w-full relative flex items-start justify-center">
              <div class="absolute -top-14">
                <Icon
                  v-if="state.error && state.error.length > 0"
                ...
                <Icon
                  ...
                  size="6rem"
                  class="text-blue-500 bg-white rounded-full"
                />
              </div>
              <span class="mt-10 mb-4 text-xl text-gray-700 text-center">
                {{
                  (state.error && state.error.length > 0) || state.isPageLoading
                    ? ""
                    : "Awesome!"
                }}
              </span>
            ...
            <div class="w-full flex justify-center pt-6 pb-4">
              <button
                ...
                class="py-1 px-6 bg-blue-500 text-white font-semibold rounded-full"
              >
                Got It!
              </button>
            </div>
  ...
  if (props.hasTour && $isRental(state.planID)) {
    ...
    try {
      ...
      if (response.data && response.data.data.length === 1) {
        ...
        if (data.noofunits > 0) await completeTour();
        else state.currentPage = 2;
