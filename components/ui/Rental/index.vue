    ...
    >
      ...
      <div class="mt-2 flex justify-between">
        <div class="flex flex-col">
          ...
          <span class="text-sm"
            >Monthly Rent: {{ $formatAmount(props.contract.montlyrent) }}</span
          >
          <span class="text-sm"
            >Advance Rent: ({{ props.contract.noofmonthadvance }})
            {{ $formatAmount(props.contract.advancerent) }}</span
          >
          <span class="text-sm"
            >Deposit: {{ $formatAmount(props.contract.deposit) }}</span
          >
          <p class="text-sm">Property: {{ props.contract.propertyList }}</p>
          <p class="text-sm">Remarks: {{ props.contract.remarks }}</p>
        </div>
        <div class="flex flex-col text-right">
          <span class="text-md font-semibold">{{
            "Contract #" + props.contract.contractno
          }}</span>
          <span class="text-sm"
            >Contract Date:
            {{ moment(props.contract.contractdate).format("MM/DD/YYYY") }}</span
          >
          <span class="text-sm"
            >Billing Sched: Every
            {{ $getNumberRank(props.contract.billingsched) }}</span
          >
          <span class="text-sm"
            >Term:
            {{
              props.contract.term && props.contract.term > 0
                ? props.contract.term + " Months"
                : "Open"
            }}</span
          >
          <span
            v-if="
              props.contract.monthlypenalty && props.contract.monthlypenalty > 0
            "
            class="text-sm"
            >Monthly: {{ props.contract.monthlypenalty }}%</span
          >
          <span class="text-sm mt-1"
            >Status:
            <span
              v-if="props.contract.status === rentalContractStatus.active"
              class="px-2 py-1 font-semibold leading-tight rounded-full text-green-700 bg-green-100"
              >Active</span
            >
            <span
              v-else
              class="px-2 py-0.5 font-semibold leading-tight rounded-full text-red-700 bg-red-100"
              >Inactive</span
            ></span
          >
        </div>
      ...
      <div class="w-full my-4 overflow-hidden rounded-lg shadow-xs">
        <div class="w-full overflow-x-auto">
          <table class="w-full whitespace-no-wrap">
            ...
            >
              <tr
                v-for="(data, index) in props.accountHistory"
                :key="index"
              ...
              >
                ...
                <td class="p-2 text-sm">
                  <a
                    v-if="data.paymentid > 0"
                    class="text-blue-700"
                    :href="`/payments/receipt?id=${data.paymentid}`"
                    target="_blank"
                    >{{ data.refno }}</a
                  >
                ...
                <td class="p-2 text-sm text-right">
                  {{
                    data.runningbalance && data.runningbalance > 0
