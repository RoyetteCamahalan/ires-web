...
const isValidRefAccount = () => {
  return (
    state.data.transtype === 0 ||
    (state.data.transtype != 0 &&
      state.data.accountid != state.data.refaccountid &&
      state.data.refaccountid > 0)
  );
};
const isValidAccountBalance = () => {
  return (
    state.data.transtype === 0 ||
    (state.data.transtype != 0 && state.currentBalance >= state.data.amount)
  );
};
          ...
          <div class="grid grid-cols-6 gap-4">
            <div class="col-span-6 sm:col-span-3">
              <div class="grid grid-cols-6 gap-4">
                <div class="col-span-6 sm:col-span-4">
                  <FormLabel
                    for="office"
                    :label="
                      state.data.transtype === 0 ? 'Office' : 'Source Office'
                    "
                  />
                  <Select
              ...
              <div v-if="state.data.transtype === 1">
                ...
                <Select
                  v-model="state.data.refaccountid"
                  :options="officeStore.activeOffices"
                  optionLabel="accountname"
                  optionValue="accountid"
                  class="w-full"
                ...
                <FormError
                  :error="
                    v$.data.refaccountid &&
                    v$.data.refaccountid.$errors &&
                    v$.data.refaccountid.$errors.length > 0
                      ? v$.data.refaccountid.$errors[0].$message
                      : null
                  "
                />
              </div>
