<template>
  <div>
    <div class="flex flex-col justify-center items-center px-6 mx-auto pt:mt-0">
      <div
        class="pt-4 w-full max-w-lg bg-white rounded-2xl shadow-lg shadow-gray-300"
      >
          ...
          <div class="space-y-8">
            ...
            <div class="w-full relative flex items-start justify-center">
              <div class="absolute -top-12">
                <img
                  v-if="state.company.logo && state.company.logo.length > 0"
                  :src="
                ...
                <div v-else class="rounded-full border-2 p-2">
                  <div
                    class="flex w-16 h-16 pb-2 text-blue-400 bg-white rounded-full items-center justify-center text-5xl font-bold"
                  >
                ...
                <label class="absolute bottom-0 right-0 mb-2 mr-2">
                  ...
                  <Icon
                    ...
                    class="w-6 h-6 text-blue-700 bg-gray-100 rounded-full p-0.5 border border-blue-700"
                  ></Icon>
                </label>
              </div>
            </div>
            <div
              v-if="!state.isEdit"
              class="px-10 py-8 space-y-1 flex flex-col items-center"
            >
            ...
            >
              ...
              <div>
                ...
                <FormError
                  :error="
                    v$.company.address &&
                    v$.company.address.$errors &&
                    v$.company.address.$errors.length > 0
                      ? v$.company.address.$errors[0].$message
                      : null
                  "
                />
              </div>
              <div>
                ...
                <FormTextField
                  name="contactno"
                  placeholder="Contact #"
                  v-model="state.company.contactno"
                ></FormTextField>
                <FormError
                  :error="
                    v$.company.contactno &&
                    v$.company.contactno.$errors &&
                    v$.company.contactno.$errors.length > 0
                      ? v$.company.contactno.$errors[0].$message
                      : null
                  "
                />
              </div>
              ...
              <button
                type="submit"
                class="py-2 px-4 w-full text-base font-medium text-center text-white bg-blue-500 hover:scale-[1.02] shadow-md shadow-gray-300 transition-transform rounded-lg"
              >
                Submit
              </button>
              <button
                type="button"
                @click="cancelEdit"
                class="py-2 px-4 w-full text-base font-medium text-center text-gray-700 bg-gray-50 hover:scale-[1.02] shadow-md shadow-gray-300 transition-transform rounded-lg"
              >
                Cancel
              </button>
            </form>
