<template>
  <div>
    <div
      class="w-full flex justify-between p-2 mt-3 mx-4 font-semibold text-blue-600"
    >
          ...
          >
            <template
              ...
              v-if="
                !(
                  state.isPageLoading ||
                  (state.bills.data && state.bills.data.length === 0)
                )
              "
            >
              <tr
                v-for="(bill, index) in state.bills.data"
                :key="index"
                class="text-gray-700"
              >
                ...
                <td class="px-4 py-3 text-sm">
                  <div class="flex flex-col">
                    <p>{{ bill.particular }}</p>
                    <span v-if="bill.datepaid" class="text-xs"
                      >Date Paid:
                      {{ moment(bill.datepaid).format("MM/DD/YYYY") }} via
                ...
                <td class="px-4 py-3 text-xs">
                  <span
                    v-if="bill.status === billStatus.open"
                  ...
                  <span
                    v-else
                    class="px-2 py-1 font-semibold leading-tight rounded-full text-gray-700 bg-gray-100"
                    >Cancelled</span
                  >
                </td>
                <td class="px-4 py-3 text-xs text-center">
                  <button
                    v-if="
                      ...
                      bill.paymentid &&
                      bill.paymentid.length > 0
                    "
                    @click="verifyPayment(bill.id)"
                    type="button"
                    class="px-2 py-1 bg-green-600 text-white rounded-lg text-xs"
                  >
                    Verify Payment
                  </button>
                  <button
                    v-else-if="bill.status === billStatus.open"
                    @click="payBill(bill.id)"
                    type="button"
                    class="px-2 py-1 bg-blue-600 text-white rounded-lg text-xs"
                  >
                    Pay Now
                  </button>
                  <button
                    @click="getInvoiceFile(bill.id)"
                    type="button"
                    class="ml-1"
                  >
                    <Icon
...
async function verifyPayment(slug) {
  ...
  try {
    ...
    } else $toastNotification("error", "", "Payment verification failed.");
  } catch (error) {
