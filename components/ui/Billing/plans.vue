<template>
  <div>
    <div class="p-2 mt-3 mx-4 font-semibold text-blue-600">My Plans</div>
      ...
      <div class="inline-block min-w-full align-middle">
        <div class="rounded-2xl shadow-lg bg-white">
          ...
          >
            ...
            >
              <tr
                v-for="(data, index) in state.plans"
                :key="index"
                class="text-gray-700"
              >
                <td class="px-4 py-3">
                  <div class="flex">
                    ...
                    <button
                      v-if="
                        data.subscriptionPlan.moduleid === appModules.rentals &&
                        data.planid < plan.rental_enterprise
                      "
                      class="ml-2 px-2 py-1 text-xs bg-blue-600 text-white rounded-lg"
                      @click="showPlanUpgrade(data.subscriptionPlan)"
                    >
                      Upgrade
                    </button>
                    <button
                      v-if="
                        data.subscriptionPlan.moduleid === appModules.survey &&
                        data.planid < plan.survey_enterprise
                      "
                      class="ml-2 px-2 py-1 text-xs bg-blue-600 text-white rounded-lg"
                      @click="showPlanUpgrade(data.subscriptionPlan)"
                    >
                ...
                <td class="px-4 py-3 text-sm">
                  <p>
                    Storage: {{ data.storage }}/{{
                      data.subscriptionPlan.storage
                    }}
                    MB
                  </p>
                  <p
                    v-if="data.subscriptionPlan.moduleid === appModules.rentals"
                  >
                    Property Limit:
                    {{ data.surveylimit ? data.surveylimit : "Unlimited" }}
                  </p>
                  <p
                    v-if="data.subscriptionPlan.moduleid === appModules.survey"
                  >
                    Survey Limit:
                    {{ data.surveylimit ? data.surveylimit : "Unlimited" }}
                  </p>
                ...
                <td class="px-4 py-3 text-xs">
                  <span
                    v-if="data.isexpired"
                    class="px-2 py-1 font-semibold leading-tight rounded-full text-red-700 bg-red-100"
                    >Expired</span
                  >
                  <span
                    v-else
...
async function upgradePlan(planID) {
  if (planID == 0) state.isShowPlanUpgrade = false;
    ...
    try {
      ...
      $toastNotification(
        "success",
        "",
        "Plan has been updated. We will email you when bill is available. Thank you!",
      );
      if (props.isUpgrade) location.replace("/billing");
