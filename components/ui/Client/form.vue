<template>
  <div>
    <form method="POST" @submit.prevent="submitForm">
      <div class="px-6 py-2">
        <div class="flex">
          <FormSwitch
            ...
            @toggleSwitch="state.isCompany = !state.isCompany"
          ></FormSwitch>
          <label class="ml-1 my-auto text-sm font-semibold text-blue-600"
            >Client is a Company</label
          >
        </div>
        <div v-if="state.isCompany" class="col-span-6 sm:col-span-3">
          <FormLabel for="companyname" label="Company Name" />
          <FormTextField
            name="companyname"
            placeholder="Company Name"
            v-model="state.client.lname"
          />
          <FormError
            :error="
              v$.client.lname &&
              v$.client.lname.$errors &&
              v$.client.lname.$errors.length > 0
                ? v$.client.lname.$errors[0].$message
                : null
            "
          />
        </div>
        <div v-if="!state.isCompany" class="grid grid-cols-6 gap-4">
          <div class="col-span-6 sm:col-span-3">
            <FormLabel for="fname" label="First Name" />
            <FormTextField
              name="fname"
              placeholder="First Name"
              v-model="state.client.fname"
            />
            <FormError
              :error="
                v$.client.fname &&
                v$.client.fname.$errors &&
                v$.client.fname.$errors.length > 0
                  ? v$.client.fname.$errors[0].$message
                  : null
              "
            />
          </div>
        ...
        <div class="grid grid-cols-6 gap-4">
          <div class="col-span-6 sm:col-span-3">
            ...
            <FormTextField
              ...
              v-model="state.client.contactno"
            />
          </div>
          <div class="col-span-6 sm:col-span-3">
        ...
        <div>
          ...
          <FormTextField
            ...
            v-model="state.client.tinnumber"
          />
        </div>
        <div>
      ...
      <div
        class="px-6 py-3 flex flex-row-reverse rounded-b border-t border-gray-200"
      >
        <FormButton type="submit" buttonStyle="primary" class="w-min"
          >Save</FormButton
        >
      </div>
    </form>
...
onMounted(async () => {
  if (props.formStatusEdit) {
    try {
      ...
      state.client.birthdate = moment(response.data.birthdate).format(
        "YYYY-MM-DD",
      );
...
const submitForm = async () => {
  ...
  if (!v$.value.$error) {
    try {
      ...
      else response = await clientService.create(state.client);
