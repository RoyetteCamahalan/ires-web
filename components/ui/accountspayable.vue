        ...
        <div class="px-2 py-4 rounded-lg bg-white">
          <div class="grid grid-cols-6 gap-4">
            <div class="col-span-6 sm:col-span-3">
              ...
              <FormSelectVendor
                v-model="state.data.vendorid"
                :defaultOption="state.vendor"
              ></FormSelectVendor>
              <FormError
                :error="
                  v$.data.vendorid &&
                  v$.data.vendorid.$errors &&
                  v$.data.vendorid.$errors.length > 0
                    ? v$.data.vendorid.$errors[0].$message
                    : null
                "
              />
            </div>
...
async function loadRecord() {
  ...
  try {
    const response = await expenseService.getAccountPayable(
      accountsPayableStore.getSelectedAP,
    );
    response.data.actualdate = moment(response.data.actualdate).format(
      "YYYY-MM-DD",
    );
...
const submit = async () => {
  ...
  if (!v$.value.$error) {
    ...
    try {
      ...
      else await expenseService.updateAccountPayable(state.data);
