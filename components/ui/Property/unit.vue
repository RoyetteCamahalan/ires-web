  ...
  <div>
    <LoadingSpinner :isActive="state.isPageLoading">
      <div class="mx-4 mt-2">
        ...
        <FormTextField
          name="name"
          placeholder="Room #/ Door #"
          v-model="state.data.propertyname"
        ></FormTextField>
        <FormError
          :error="
            v$.data.propertyname &&
            v$.data.propertyname.$errors &&
            v$.data.propertyname.$errors.length > 0
              ? v$.data.propertyname.$errors[0].$message
              : null
          "
        />
        ...
        <FormNumberField
          ...
          v-model="state.data.monthlyrent"
        ></FormNumberField>
        <FormError
          :error="
            v$.data.monthlyrent &&
            v$.data.monthlyrent.$errors &&
            v$.data.monthlyrent.$errors.length > 0
              ? v$.data.monthlyrent.$errors[0].$message
              : null
          "
        />
      ...
      <div class="px-4">
        ...
        <div
          :class="
            'flex ' +
            (state.isMaxQuota || props.isOnBoarding
              ? 'justify-between'
              : 'flex-row-reverse')
          "
        >
          <FormButton
            v-if="state.isMaxQuota"
            buttonStyle="default"
            class="text-sm"
            @click="navigateTo('/billing?upgrade=1')"
            >Upgrade Plan</FormButton
          >
          <FormButton
            v-if="props.isOnBoarding"
            buttonStyle="default"
            class="text-sm"
            @click="$emit('onSkipTour')"
            >Skip Tutorial</FormButton
          >
...
async function submit() {
  ...
  if (!v$.value.$error) {
    try {
      ...
      else await projectService.UpdateRentalUnit(state.data);
...
async function loadRecord() {
  ...
  try {
    ...
    if (state.showIsActive)
      state.data.isactive =
        response.data.status === rentalPropertyStatus.vacant;
  } catch (error) {
    state.error = error.message;
  }
  state.isPageLoading = false;
