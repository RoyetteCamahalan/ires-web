    ...
    <LoadingSpinner :isActive="state.isPageLoading">
      <div class="mx-4 mt-2">
        ...
        <div class="relative">
          <FormSelectOtherFees
            name="otherfeeid"
            :default-option="state.otherfee"
            v-model="state.data.otherfeeid"
            class="pr-16"
          ></FormSelectOtherFees>
          <a
        ...
        <FormError
          :error="
            v$.data.otherfeeid &&
            v$.data.otherfeeid.$errors &&
            v$.data.otherfeeid.$errors.length > 0
              ? v$.data.otherfeeid.$errors[0].$message
              : null
          "
        />
        ...
        <LoadingSpinner :isActive="state.isFetchingAmount">
          <FormNumberField
            name="amount"
            placeholder="Amount"
            v-model="state.data.chargeamount"
          />
        </LoadingSpinner>
        <FormError
          :error="
            v$.data.chargeamount &&
            v$.data.chargeamount.$errors &&
            v$.data.chargeamount.$errors.length > 0
              ? v$.data.chargeamount.$errors[0].$message
              : null
          "
        />
        ...
        <div v-if="state.hasInterest">
          <FormLabel
            for="interestpercentage"
            label="Monthly Interest Percentage"
          />
          <FormNumberField
            name="interestpercentage"
            placeholder="Interest Percentage (e.g. 3)"
            v-model="state.data.interestpercentage"
          />
          <FormError
            :error="
              v$.data.interestpercentage &&
              v$.data.interestpercentage.$errors &&
              v$.data.interestpercentage.$errors.length > 0
                ? v$.data.interestpercentage.$errors[0].$message
                : null
            "
          />
        </div>
      ...
      <div class="px-4">
        ...
        <div
          class="flex"
          :class="
            (props.formStatus === 0 && 'flex-row-reverse',
            props.formStatus != 0 && 'justify-between')
          "
        >
          <FormButton
            v-if="props.formStatus != 0"
            buttonStyle="danger"
            class="text-sm"
            @click="removeCharge"
            >Remove this Charge</FormButton
          >
          <FormButton buttonStyle="primary" class="text-sm" @click="submit"
            >Submit</FormButton
          >
        </div>
      </div>
...
onMounted(() => {
  if (props.formStatus === 0) state.data.propertyid = 0;
  else if (props.formStatus === 1) loadRecord();
});

watch(
  () => state.data.otherfeeid,
  async (newValue) => {
    if (
      props.formStatus === 0 &&
      state.data.otherfeeid &&
      state.data.otherfeeid > 0
    )
...
async function loadDefaultAmount() {
  ...
  try {
    const response = await otherChargeService.GetOtherFee(
      state.data.otherfeeid,
    );
...
async function submit() {
  ...
  if (!v$.value.$error) {
    try {
      if (!state.hasInterest) state.data.interestpercentage = 0;
      if (props.formStatus === 0)
        await rentalService.postOtherCharge(state.data);
      else await rentalService.updateOtherCharge(state.data);
...
async function loadRecord() {
  ...
  try {
    ...
    state.otherfee = {
      value: response.data.otherFee.id,
      label: response.data.otherFee.name,
    };
    state.data.otherfeeid = response.data.otherfeeid;
    state.data.chargedate = moment(response.data.chargedate).format(
      "YYYY-MM-DD",
    );
