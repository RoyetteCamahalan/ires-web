    ...
    <LoadingSpinner :isActive="state.isPageLoading">
      <div class="mx-4 mt-2">
        ...
        <FormTextField
          ...
          v-model="state.bank.name"
        ></FormTextField>
        <FormError
          :error="
            v$.bank.name &&
            v$.bank.name.$errors &&
            v$.bank.name.$errors.length > 0
              ? v$.bank.name.$errors[0].$message
              : null
          "
        />
...
onMounted(() => {
  if (props.formStatus === 0) state.bank.bankid = 0;
...
async function submit() {
  ...
  if (!v$.value.$error) {
    try {
      ...
      else response = await bankService.update(state.bank);
