...
const isValidAccountBalance = () => {
  return (
    props.formStatus === 1 ||
    !state.data.usepettycash ||
    (props.formStatus === 0 &&
      state.data.usepettycash &&
      state.currentBalance >= state.data.amount)
  );
};
const validators = computed(() => {
  return {
    ...
    currentBalance: {
      isValidRefAccount: helpers.withMessage(
        "Insufficient Balance, Please cash in or uncheck `From Petty Cash`",
...
onMounted(() => {
  if (props.formStatus === 1) loadRecord();
});

watch(
  () => state.data.accountid,
  () => {
    if (state.data.accountid > 0) getOfficeBalance();
  },
);
...
const submit = async () => {
  ...
  if (!v$.value.$error) {
    state.isPageLoading = true;
    try {
      if (props.formStatus === 0) await expenseService.create(state.data);
      else await expenseService.update(state.data);
      navigateTo("/expenses");
    } catch (error: any) {
      state.error = error.message;
    }
    ...
    <LoadingSpinner :isActive="state.isPageLoading">
      <div class="mx-4 mt-2">
        ...
        <div class="px-2 py-4 rounded-lg bg-white">
          <div
            class="flex mt-1"
            title="Deduct this expense from Petty Cash fund"
          >
            <FormCheckBox
              v-model="state.data.usepettycash"
              class="my-auto"
            ></FormCheckBox>
            <FormLabel
              for="frompettycash"
            ...
            <Icon
              ...
              class="w-5 h-5 ml-1 my-auto text-blue-500"
            ></Icon>
          </div>
          <div class="grid grid-cols-6 gap-4">
            <div class="col-span-6 sm:col-span-3">
              ...
              <div class="relative">
                <FormSelectVendor
                  v-model="state.data.payeeid"
                  :defaultOption="state.vendor"
                  class="pr-16"
                ></FormSelectVendor>
                <button
              ...
              <FormError
                :error="
                  v$.data.payeeid &&
                  v$.data.payeeid.$errors &&
                  v$.data.payeeid.$errors.length > 0
                    ? v$.data.payeeid.$errors[0].$message
                    : null
                "
              />
            </div>
            <div class="col-span-6 sm:col-span-3">
              <div class="grid grid-cols-6 gap-4">
                <div
                  class="col-span-6"
                  :class="
                    user.companyid !== 15 || user.isappsysadmin
                      ? 'sm:col-span-4'
                      : ''
                  "
                >
                  ...
                  <FormError
                    :error="
                      v$.data.accountid &&
                      v$.data.accountid.$errors &&
                      v$.data.accountid.$errors.length > 0
                        ? v$.data.accountid.$errors[0].$message
                        : null
                    "
                  />
                </div>
                ...
                >
                  ...
                  <LoadingSpinner :isActive="state.isFetchingBalance">
                    <FormNumberField
                      ...
                      class="text-right cursor-not-allowed bg-blue-50"
                  ...
                  <FormError
                    :error="
                      v$.currentBalance &&
                      v$.currentBalance.$errors &&
                      v$.currentBalance.$errors.length > 0
                        ? v$.currentBalance.$errors[0].$message
                        : null
                    "
                  />
                </div>
          ...
          <div class="grid grid-cols-6 gap-4">
            <div class="col-span-6 sm:col-span-3">
              ...
              <div class="relative">
                <FormSelectExpensetypes
                  v-model="state.data.expensetypeid"
                ></FormSelectExpensetypes>
                <a
              ...
              <FormError
                :error="
                  v$.data.expensetypeid &&
                  v$.data.expensetypeid.$errors &&
                  v$.data.expensetypeid.$errors.length > 0
                    ? v$.data.expensetypeid.$errors[0].$message
                    : null
                "
              />
            </div>
            <div class="col-span-6 sm:col-span-3">
              ...
              <FormTextField
                name="refno"
                placeholder="Ref #"
                v-model="state.data.refno"
              />
              <FormError
                :error="
                  v$.data.refno &&
                  v$.data.refno.$errors &&
                  v$.data.refno.$errors.length > 0
                    ? v$.data.refno.$errors[0].$message
                    : null
                "
              />
            </div>
          ...
          <div class="grid grid-cols-6 gap-4">
            ...
            <div class="col-span-6 sm:col-span-3">
              ...
              <FormNumberField
                name="amount"
                placeholder="Amount"
                v-model="state.data.amount"
              />
              <FormError
                :error="
                  v$.data.amount &&
                  v$.data.amount.$errors &&
                  v$.data.amount.$errors.length > 0
                    ? v$.data.amount.$errors[0].$message
                    : null
                "
              />
            </div>
          ...
          <div class="mt-4 text-sm text-gray-600">
            * You can manage
            <a href="/masterfiles/vendors" class="text-blue-500"
              >Vendor/Supplier</a
            >,
            <a href="/masterfiles/offices" class="text-blue-500">Offices</a> and
            <a href="/masterfiles/expensetypes" class="text-blue-500"
              >Expense Types</a
            >
            in `Master Files Menu`
          </div>
        </div>
      </div>
