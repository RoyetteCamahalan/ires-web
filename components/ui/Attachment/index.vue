<template>
  <div>
    <div
      class="flex flex-col justify-center items-center px-6 my-auto mx-auto pt:mt-0"
    >
      <div
      ...
      >
        <div class="space-y-8">
          ...
          <LoadingSpinner :isActive="state.isPageLoading">
            ...
            <div
              v-if="state.files && state.files.length > 0"
              class="grid grid-cols-4 gap-4 p-3"
            >
              <div
              ...
              >
                <div class="flex flex-row-reverse">
                  <Menu as="div" class="relative inline-block text-left">
                    <div>
                      ...
                      >
                        <Icon
                          name="ic:outline-expand-circle-down"
                          class="w-4 h-4"
                        ></Icon>
                      </MenuButton>
                    </div>
                ...
                />
                <img
                  v-else-if="file.filetype === 1"
                  src="~/assets/img/pdf_file.svg"
                  class="w-16 h-16 mx-auto"
                />
                <img
                  v-else-if="file.filetype === 2"
                ...
                />
                <span class="text-xs">{{
                  file.documentname.substring(0, 12)
                }}</span>
              </div>
            </div>
...
async function getFiles() {
  ...
  try {
    const response = await fileService.get(
      props.typeID,
      props.attachableID,
      props.lotID,
    );
...
function downloadFile(file) {
  $downloadFile(
    "wwwroot/attachments/" + user.companyid + "/" + file.filename,
    file.documentname,
  );
}
