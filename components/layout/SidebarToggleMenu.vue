<template>
  <div class="bg-white rounded-b-lg">
    ...
    >
      ...
      >
        <Icon :name="props.iconname" class="w-4 h-4"></Icon>
      </div>
      <span class="ml-3 text-dark-500 text-sm">{{ props.title }}</span>
      <svg
        sidebar-toggle-item=""
    ...
    >
      <ul class="pb-2 pt-1" :class="!state.toggleOpen && 'hidden'">
        <slot></slot>
      </ul>
    </transition>
  </div>
...
const props = defineProps({
  ...
  iconname: {
    type: String,
    required: false,
    default: "material-symbols:verified-user",
  },
  toggleOpen: {
    type: Boolean,
    required: false,
