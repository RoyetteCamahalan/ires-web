<template>
  <aside
    id="sidebar"
    class="flex fixed top-0 left-0 flex-col flex-shrink-0 h-full pt-16 w-64 duration-200 lg:flex transition-width"
    :class="!sidebarOpen && !openHoverState ? ' lg:w-0' : ' z-20 lg:w-64'"
    aria-label="Sidebar"
              ...
              >
                ...
                >
                  <li class="ml-2 mt-1">
                    ...
                    >
                      <div
                        class="hidden w-2 h-2 bg-blue-500 rounded-full my-auto mr-1"
                      ></div>
                      Register and Login
                    </a>
                  </li>
...
function toggleMenu(menu) {
  if (!state.pageCurrent) state.pageCurrent = { [menu]: true };
  else state.pageCurrent[menu] = !state.pageCurrent[menu];
}
</script>
