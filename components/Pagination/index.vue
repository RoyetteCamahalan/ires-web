<template>
  <div
    v-if="!(isLoading || (data && data.data && data.data.length === 0))"
  ...
  >
    <span
      class="w-full col-span-12 sm:col-span-4 sm:text-left text-center sm:my-auto"
    >
      {{ data.info }}
    </span>
    <!-- Pagination -->
    <div
      class="w-full col-span-12 sm:col-span-8 mt-2 sm:mt-auto sm:flex sm:justify-end text-center"
    >
            ...
            <button
              ...
              :class="
                currentPage == page.page
                  ? 'text-white transition-colors duration-150 bg-blue-600 border border-r-0 border-blue-600'
                  : ''
              "
  ...
  setup(props) {
    ...
    watch(
      ...
      (data) => {
        if (data && data.data) {
          state.pages = [];
          let leftPointer = data.currentPage - 2;
          let rightPointer = data.currentPage + 2;
          ...
          while (pageNumber <= data.totalPages) {
            if (
              ...
              (pageNumber == rightPointer &&
                rightPointer == data.totalPages - 1) ||
              pageNumber == data.totalPages
            ) {
              state.pages.push({ page: pageNumber, text: pageNumber });
            } else if (
              pageNumber == leftPointer ||
