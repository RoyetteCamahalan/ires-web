...
class DashboardService extends BaseAPIService {
  ...
  async getSurvey(): Promise<any> {
    ...
    return await this.request(
      `/main/getsurveydashboard?currentDate=${moment(currentDate).format("YYYY-MM-DD")}`,
      "GET",
    );
  }
  async getRental(): Promise<any> {
    var currentDate = new Date();
    return await this.request(
      `/main/getrentaldashboard?currentDate=${moment(currentDate).format("YYYY-MM-DD")}`,
      "GET",
    );
  }
  ...
  async getCalendarEvents(startDate: number, endDate: number): Promise<any> {
    return await this.request(
      `/main/getcalendarevents?startDate=${startDate}&&endDate=${endDate}`,
      "GET",
    );
  }
