...
class PaymentService extends BaseAPIService {
  async getPayables(
    ...
    return await this.request(
      `/payment/getpayables?clientID=${clientID}&currentPage=${currentPage}&search=${search}`,
      "GET",
    );
  }
  async getReceiptNo(receiptType: number): Promise<any> {
    return await this.request(
      `/payment/getreceiptno?receiptType=${receiptType}`,
      "GET",
    );
  }
  ...
  async getCreditNotes(params: object): Promise<any> {
    return await this.request(`/payment/getcreditnotes`, "GET", params);
  }
}
