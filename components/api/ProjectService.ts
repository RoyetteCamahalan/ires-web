...
class ProjectService extends BaseAPIService {
  ...
  async GetRentalProperties(currentPage: number, search: string): Promise<any> {
    return await this.request(
      `/project/getrentalproperties/?currentPage=${currentPage}&search=${search}`,
      "GET",
    );
  }
  ...
  async GetRentalUnits(
    ...
    return await this.request(
      `/project/getrentalunits/?projectID=${projectID}&currentPage=${currentPage}&search=${search}`,
      "GET",
    );
  }
  ...
  async GetAvailableRentalUnits(
    currentPage: number,
    search: string,
  ): Promise<any> {
    return await this.request(
      `/project/getavailablerentalunits/?currentPage=${currentPage}&search=${search}`,
      "GET",
    );
  }
}
