    ...
    <LoadingSpinner :isActive="state.isPageLoading">
      ...
      >
        ...
        <div class="mt-1 text-sm text-center">
          From: {{ moment(props.startDate).format("MM/DD/YYYY") }} To:
          {{ moment(props.endDate).format("MM/DD/YYYY") }}
        </div>
        <div class="mt-1 text-sm text-center">
          Account Name:
          <span class="font-semibold">{{ state.accountname }}</span>
        </div>
        <div
          class="w-full my-4 rounded-lg shadow-xs"
          :class="!props.isPrint && 'overflow-visible'"
        >
