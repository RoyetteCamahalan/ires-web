<template>
  <div>
    <NuxtLayout name="admin" title="Payments">
      ...
      <div
        class="flex flex-col my-3 mx-4 shadow-gray-200 rounded-lg shadow-md overflow-x-hidden"
      >
            ...
            >
              <template
                ...
                v-if="
                  !(
                    state.isPageLoading ||
                    (state.payments.data && state.payments.data.length === 0)
                  )
                "
              >
                <tr
                  v-for="(data, index) in state.payments.data"
                  :key="index"
                  class="text-gray-700"
                >
                  <td class="px-4 py-3">
                    ...
                    <div class="flex text-xs">
                      ...
                      <span
                        v-if="data.status === paymentStatus.void"
                        class="px-2 py-1 ml-1 font-semibold leading-tight rounded-full text-red-700 bg-red-100"
                        >Void</span
                      >
                      <span
                        v-else-if="data.status === paymentStatus.refunded"
                        class="px-2 py-1 ml-1 font-semibold leading-tight rounded-full text-gray-700 bg-gray-100"
                        >Refunded</span
                      >
                    </div>
                  </td>
                    ...
                    <div class="flex justify-center">
                      ...
                      >
                        <button
                          ...
                          @click="
                            navigateTo('/payments/receipt?id=' + data.paymentid)
                          "
                          class="group flex w-full items-center rounded-md px-2 py-2 text-sm hover:bg-gray-100"
                        >
                          Print Receipt
      ...
      <ModalEmpty title="" :isShow="state.modalIsShowOverride">
        <UiUserOverride
          :action="'Void Payment: ' + state.selectedPaymentID"
          @AfterOverride="modalCloseOverride"
          :has-remarks="true"
        ></UiUserOverride>
      </ModalEmpty>
      <Modal
        title="Payment Settings"
        :isShow="state.modalIsShowSettings"
        @modalClose="state.modalIsShowSettings = false"
      >
        <UiCompanySetting
          @close="state.modalIsShowSettings = false"
        ></UiCompanySetting>
      </Modal>
    </NuxtLayout>
  </div>
  ...
  if (data && data.success) {
    ...
    try {
      await paymentService.voidPayment({
        paymentid: state.selectedPaymentID,
        voidremarks: data.remarks,
      });
