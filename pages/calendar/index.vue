    ...
    <NuxtLayout name="admin" title="My Calendar">
      <div class="m-2">
        <div class="w-full mt-5 rounded-lg border border-gray-300 bg-white">
          <div
            class="w-full px-4 py-3 flex justify-between bg-blue-50 rounded-t-lg"
          >
            ...
            <div class="flex">
              <MenuPopOver :label="state.monthName" position="right-0">
                <button
                  v-for="(data, index) in state.months"
                  :key="index"
              ...
              <MenuPopOver :label="state.year" class="ml-2" position="right-0">
                <button
                  v-for="(data, index) in state.years"
                  :key="index"
          ...
          <div class="w-full">
            <div class="grid grid-cols-7 bg-gray-200 gap-px border-b">
              <div
                class="col-span-1 px-4 py-2 bg-white text-center text-sm font-semibold"
              >
            ...
            <LoadingSpinner :is-active="state.isPageLoading">
              <div
                :class="[
                  'grid grid-cols-7 bg-gray-200 gap-px text-sm',
                  state.calendarDays.length > 35
                    ? 'grid-rows-6'
                    : 'grid-rows-5',
                ]"
              >
                <button
                  v-for="(data, index) in state.calendarDays"
                  :key="index"
                  :class="[
                    'relative px-3 py-2',
                    data.isactive ? 'bg-white' : 'bg-gray-50',
                  ]"
                ...
                >
                  <div class="flex mb-2">
                    <div
                      :class="[
                        'flex justify-center w-6 h-6 ml-auto sm:ml-0',
                        data.events.length == 0
                          ? 'text-gray-500'
                          : 'font-semibold text-white bg-blue-500 rounded-full items-center',
                      ]"
                    >
                      {{ data.date.getDate() }}
                    </div>
                  ...
                  <div
                    ...
                    :key="index"
                    class="w-full hidden sm:inline-block"
                  >
                    <p
                      v-if="index < 3"
                      class="text-blue-700 text-xs font-semibold text-left truncate"
                    >
                      {{ event.title }}
                    </p>
                    <p
                  ...
                  <span class="flex sm:hidden">
                    ...
                    >
                      <span
                        v-if="index < 3"
                        class="w-1.5 h-1.5 bg-blue-500 rounded-full mr-0.5"
                      ></span>
                    </span>
                  </span>
                </button>
        ...
        <div
          id="dailyevents"
          class="w-full mt-5 rounded-lg border border-gray-300 bg-white"
        >
          <div>
            <div
              class="w-full px-4 py-2 flex justify-between bg-blue-50 rounded-t-lg"
            >
              <h1 class="font-semibold">
                Events -
                {{
                  state.selectedDay
                    ? moment(state.selectedDay.date).format("MMMM DD, YYYY")
                    : "No date selected"
                }}
              </h1>
            </div>
            <div class="w-full">
              <div
                v-if="state.selectedDay && state.selectedDay.events.length > 0"
                class="m-2 md:m-4"
              >
                <div
                  v-for="(data, index) in state.selectedDay.events"
                  :key="index"
                  class="flex border-b p-2 md:m-4"
                >
                  <div
                  ...
                  <div class="flex flex-col">
                    ...
                    <div class="flex text-sm">
                      <span>Remarks:</span>
                      <p
                        v-if="data.moduleid === appModules.survey"
                        class="ml-2 italic"
                      >
                        test
                      </p>
              ...
              <div v-else class="py-4 text-sm italic text-center">
                No events found!
              </div>
            </div>
          </div>
...
async function fetchEvents() {
  ...
  try {
    ...
    const edate = moment(
      state.calendarDays[state.calendarDays.length - 1].date,
    ).format("YYYY-MM-DD");
...
function generateCalendar() {
  ...
  if (sdate.getDay() > 0) sdate.setDate(sdate.getDate() - sdate.getDay());

  state.calendarDays = [];
  while (sdate.getMonth() < state.month) {
    var event = {
      date: new Date(sdate),
      isactive: sdate.getMonth() === state.month - 1,
      events: [],
    };
