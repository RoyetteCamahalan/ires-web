<template>
  <div>
    <NuxtLayout name="admin" title="Account Payables">
              ...
              >
                ...
                >
                  ...
                  <td class="px-4 py-3 text-xs text-center">
                    <span
                      v-if="data.status === 0"
                      class="px-2 py-1 font-semibold leading-tight rounded-full bg-green-100"
                      >Approved</span
                    >
                    <span
                      v-else
      ...
      <ModalEmpty title="" :isShow="state.modalIsShowOverride">
        <UiUserOverride
          :action="'Void Accounts Payable: ' + state.selectedDataID"
...
const loadList = async (search) => {
  ...
  try {
    ...
    const response = await expenseService.getAccountPayables(params);
...
const updateRecord = (value) => {
  accountsPayableStore.setSelectedAP(value);
  navigateTo("/accountspayable/edit");
};
...
const modalCloseOverride = async (success) => {
  ...
  if (success) {
    state.isPageLoading = true;
    try {
      await expenseService.VoidAccountPayable({ id: state.selectedDataID });
