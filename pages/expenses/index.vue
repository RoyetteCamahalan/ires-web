  ...
  <div>
    <NuxtLayout name="admin" title="Expenses">
      <div class="flex flex-col mt-2 mx-4">
        <div class="w-full flex justify-between flex-col sm:flex-row">
          ...
          <div class="flex justify-between flex-1">
            <MenuDropDown2
              class="ml-0 sm:ml-2 my-auto mt-1"
              label="Print"
              :has-icon="true"
              position="left-0"
              width="w-48"
      ...
      <ModalEmpty title="" :isShow="state.modalIsShowOverride">
        <UiUserOverride
          :action="'Void Expense: ' + state.selectedDataID"
...
function PrintReport(type) {
  if (state.mainList && state.mainList.data && state.mainList.data.length > 0) {
    ...
    if (type === 1) window.open("/reports/expense", "_blank");
    else if (type === 2)
      window.open("/reports/expense/byexpensetype", "_blank");
    else if (type === 3) window.open("/reports/expense/byoffice", "_blank");
  } else $toastNotification("error", "", "Report is empty");
}
</script>
