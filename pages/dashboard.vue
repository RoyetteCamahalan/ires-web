<template>
  <div>
    <NuxtLayout name="admin" title="Dashboard">
      <DashboardSurveying
        v-if="$isSurveying(state.planID)"
        class="my-5"
      ></DashboardSurveying>
      <DashboardRental
        v-if="$isRental(state.planID)"
        class="my-5"
      ></DashboardRental>
      <DashboardFinance class="my-5"></DashboardFinance>
      <UiOnboarding
        :hasTour="state.hasTour"
        :isShow="state.isShowWelcome"
        @onHideTour="hideTour"
      ></UiOnboarding>
    </NuxtLayout>
  </div>
...
async function loadRecord() {
  try {
    ...
    if (response.data.totalProperties == 0) state.hasTour = true;
  } catch (error) {}
}
function hideTour() {
