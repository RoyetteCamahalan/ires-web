    ...
    <div>
        <NuxtLayout name="admin" title="Credit Types">
            ...
            <Modal :title="state.modalTitle" :isShow="state.modalShow" @modalClose="modalClose">
                <UiCreditMemoType @modalClose="modalClose" :isFormEdit="state.formStatus" :selectedDataID="state.selectedDataID"></UiCreditMemoType>
            </Modal>
        </NuxtLayout>
    </div>
</template>
...
const loadList = async (search) =>{
    ...
    try{
        const response = await creditNoteService.getCreditTypes(state.currentPage, search, true)
