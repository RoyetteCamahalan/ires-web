    ...
    <div>
        <NuxtLayout name="admin" title="Other Fees">
            ...
            <Modal :title="state.modalTitle" :isShow="state.modalShow" @modalClose="modalClose">
                <UiOtherFee @modalClose="modalClose" :formStatus="state.formStatus" :selectedDataID="state.selectedDataID"></UiOtherFee>
            </Modal>
        </NuxtLayout>
    </div>
</template>
...
const loadList = async (search) =>{
    ...
    try{
        const response = await otherChargeService.GetOtherFees(state.currentPage, true, search)
