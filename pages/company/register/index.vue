<template>
  <div>
    <div
      v-if="state.newCompanyID === 0"
      class="flex items-center min-h-screen p-6 bg-gray-50"
    >
      <div
        class="flex-1 h-full max-w-lg mx-auto overflow-hidden bg-white rounded-lg shadow-xl"
      >
              ...
              <div class="w-full">
                ...
                <form method="POST" @submit.prevent="Submit">
                  ...
                  <FormError
                    :error="
                      v$.company.name &&
                      v$.company.name.$errors &&
                      v$.company.name.$errors.length > 0
                  ...
                  <div class="grid grid-cols-6 gap-4">
                    <div class="col-span-6 sm:col-span-3">
                      ...
                      <FormTextField
                        ...
                        v-model="state.company.adminfirstname"
                      />
                      <FormError
                        :error="
                          v$.company.adminfirstname &&
                          v$.company.adminfirstname.$errors &&
                          v$.company.adminfirstname.$errors.length > 0
                            ? v$.company.adminfirstname.$errors[0].$message
                            : null
                        "
                      />
                    </div>
                    <div class="col-span-6 sm:col-span-3">
                      ...
                      <FormTextField
                        ...
                        v-model="state.company.adminlastname"
                      />
                      <FormError
                        :error="
                          v$.company.adminlastname &&
                          v$.company.adminlastname.$errors &&
                          v$.company.adminlastname.$errors.length > 0
                            ? v$.company.adminlastname.$errors[0].$message
                            : null
                        "
                      />
                    </div>
                  ...
                  <div class="flex">
                    <FormLabel for="email" label="Email" /><span
                      class="block text-xs mt-2.5 ml-1 text-gray-500"
                      >(Verification email will be sent here)</span
                    >
                  </div>
                  <FormTextField
                    name="email"
                  ...
                  <FormError
                    :error="
                      v$.company.email &&
                      v$.company.email.$errors &&
                      v$.company.email.$errors.length > 0
                  ...
                  <FormPasswordField
                    ...
                    v-model="state.company.password"
                  />
                  <FormError
                    :error="
                      v$.company.password &&
                      v$.company.password.$errors &&
                      v$.company.password.$errors.length > 0
                        ? v$.company.password.$errors[0].$message
                        : null
                    "
                  />
                  ...
                  <FormSelect
                    :options="state.plans"
                    v-model="state.company.planid"
                    :canClear="false"
                  ></FormSelect>
                  <FormError
                    :error="
                      v$.company.planid &&
                      v$.company.planid.$errors &&
                      v$.company.planid.$errors.length > 0
                        ? v$.company.planid.$errors[0].$message
                        : null
                    "
                  />
    ...
    <div
      v-else
      class="fixed inset-x-0 h-full flex items-start bg-black bg-opacity-50 sm:items-center sm:justify-center"
    >
      <!-- Modal -->
      <div
        class="w-full mx-2 px-6 py-4 bg-white rounded-b-lg sm:rounded-lg sm:m-4 sm:max-w-xl"
        role="dialog"
        id="modal"
      >
          ...
          <LoadingSpinner :is-active="state.isPageLoading">
            <div class="w-full relative flex items-start justify-center">
              <div class="absolute -top-14">
                <Icon
                  ...
                  size="6rem"
                  class="text-green-400 bg-white rounded-full"
                />
              </div>
            ...
            <div>
              <p class="text-sm text-gray-500 text-center">
                We have sent an email confirmation link to your registered
                email.
              </p>
            </div>
            <div class="flex px-4 mt-4 justify-between flex-col sm:flex-row">
              <button
                class="border px-3 py-1 rounded-md text-gray-700 bg-gray-100 text-sm"
                @click="SendConfirmationLink()"
              >
                Re-Send Link
              </button>
              <button
                class="border px-3 py-1 mt-1 rounded-md text-white bg-blue-500"
...
const state = reactive({
  ...
  company: {
    ...
    email: "",
    password: "",
    planid: subscriptionStore.getSelectedPlanID
      ? subscriptionStore.getSelectedPlanID
      : null,
  },
  isAgree: false,
  plans: [
  ...
  if (!v$.value.$error) {
    ...
    try {
      ...
      $toastNotification(
        "success",
        "Success!",
        "Email confirmation link sent to your email",
      );
