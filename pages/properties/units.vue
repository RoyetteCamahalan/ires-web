            ...
            >
              <template
                ...
                v-if="
                  !(
                    state.isPageLoading ||
                    (state.mainList.data && state.mainList.data.length === 0)
                  )
                "
              >
                <tr
                  v-for="(data, index) in state.mainList.data"
                  :key="index"
                  class="text-gray-700"
                >
                  ...
                  <td class="px-4 py-3 text-sm text-center">
                    <a
                      ...
                      :href="'/rentals/contract?ref=' + data.contract_id"
                      class="text-blue-600"
                      >{{ data.tenant }}</a
                    >
                  </td>
                  <td class="px-4 py-3 text-xs text-center">
                    <span
                      v-if="data.status === rentalPropertyStatus.vacant"
                      class="px-2 py-1 font-semibold leading-tight rounded-full text-green-700 bg-green-100"
                      >Vacant</span
                    >
                    <span
                      v-else-if="data.status === rentalPropertyStatus.occupied"
                      class="px-2 py-1 font-semibold leading-tight rounded-full text-blue-700 bg-green-100"
                      >Occupied</span
                    >
                    <span
                      v-else
...
const loadList = async (search) => {
  state.isPageLoading = true;
  try {
    const response = await projectService.GetRentalUnits(
