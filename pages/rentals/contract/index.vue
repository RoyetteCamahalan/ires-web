  ...
  <div>
    <NuxtLayout name="admin">
      <div class="p-3 h-screen bg-gray-50">
            ...
            <div class="flex">
              ...
              >
                ...
                <button
                  @click="postOtherFee"
                  class="group flex justify-between w-full items-center rounded-md px-2 py-2 text-sm hover:bg-gray-100"
                  title="Add fees other than monthly rental"
                >
                  Post Other Fees
                  <Icon
                ...
                >
                  Attachments
                  <span v-if="state.fileCount" class="ml-1 text-blue-500"
                    >({{ state.fileCount }})</span
                  >
                </button>
                <button
                ...
                >
                  {{
                    state.contract.status === rentalContractStatus.active
                      ? "Terminate Contract"
                      : "Reactivate Contract"
                  }}
                </button>
        ...
        <Modal
          :title="state.ModalOtherFeesVisibleTitle"
          :isShow="state.isModalOtherFeesVisible"
          @modalClose="onCloseModalOtherFees"
        >
          <UiOtherCharge
            @modalClose="onCloseModalOtherFees"
            :formStatus="state.modalOtherFeesFormStatus"
            :selectedDataID="state.selectedOtherChargeID"
            :contractid="state.contractID"
          ></UiOtherCharge>
        </Modal>
        <ModalEmpty :isShow="state.modalIsShowAttachment">
          <UiAttachment
            :typeID="14"
            :attachableID="state.contractID"
            @closeAttachment="closeAttachment"
          ></UiAttachment>
        </ModalEmpty>
        <ModalEmpty title="" :isShow="state.modalIsShowEmail">
          <UiSetEmail
            :email="state.contract.client.email"
            @modalClose="closeSetEmail"
          ></UiSetEmail>
        </ModalEmpty>
        <ModalGeneric
          :title="state.ModalGenericTitle"
          :isShow="state.ModalGenericIsShow"
          type="terminaterental"
          @modalClose="closeGeneric"
        ></ModalGeneric>
      </div>
    </NuxtLayout>
  </div>
