  ...
  <div>
    <NuxtLayout name="admin" title="Rental Contracts">
      <div class="flex flex-col my-2 mx-4">
        <div class="w-full flex justify-between flex-col sm:flex-row">
          <div class="w-full sm:w-80 flex items-center">
            <label class="text-sm mr-2">Filter: </label
            ><FormSelect
              :options="state.filterOptions"
              :searchable="false"
              :canClear="false"
              v-model="state.filterByID"
            ></FormSelect>
          </div>
              ...
              >
                ...
                >
                  <td class="px-4 py-3 text-sm text-center">
                    <a
                      :href="'rentals/contract?ref=' + data.contractid"
                      class="text-blue-600"
                      >{{ data.contractno }}</a
                    >
                  </td>
                  ...
                  <td class="px-4 py-3 text-xs text-center">
                    <span
                      v-if="data.status === rentalContractStatus.active"
                    ...
                    <div class="flex items-center">
                      ...
                      >
                        <button
                          @click="
                            navigateTo(
                              'rentals/contract?ref=' + data.contractno,
                            )
                          "
      ...
      <ModalEmpty title="" :isShow="state.modalIsShowAttachment">
        <UiAttachment
          :typeID="14"
          :attachableID="state.selectedContractID"
...
const loadList = async (search) => {
  ...
  try {
    const response = await rentalService.get(
      state.currentPage,
      search,
      state.filterByID,
    );
